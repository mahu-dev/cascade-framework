package io.github.cascade.cache.core.simple;
...
public class AutoRefreshCache<K, V> implements Cache<K, V> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AutoRefreshCache.class);
    ...
    public AutoRefreshCache(Cache<K, V> delegate,
        ...
        LOGGER.info("AutoRefreshCache 已创建，默认刷新间隔: {}秒，自动启动: {}",
    ...
    private void trackKeyForRefresh(K key) {
        ...
        try {
            ...
            LOGGER.debug("键 [{}] 已自动添加到刷新监控列表，刷新间隔: {}秒", key, refreshInterval);

            // 自动启动刷新器（如果还未启动）
            if (autoStart && !refresher.isRunning()) {
                refresher.start();
                LOGGER.info("刷新器已自动启动");
            }
        } catch (Exception e) {
            LOGGER.warn("自动追踪键 [{}] 失败，继续执行", key, e);
        }
    }
    ...
    public void setKeyRefreshInterval(K key, long refreshIntervalSeconds) {
        ...
        if (trackedKeys.contains(key)) {
            ...
            LOGGER.info("键 [{}] 的刷新间隔已更新为: {}秒", key, refreshIntervalSeconds);
        }
    }
