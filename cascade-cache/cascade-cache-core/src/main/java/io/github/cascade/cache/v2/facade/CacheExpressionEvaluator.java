...
public class CacheExpressionEvaluator {
    ...
    public boolean evaluateBoolean(String expressionString,
        ...
        try {
            Expression expression = getOrCompileExpression(expressionString);
            EvaluationContext context = createEvaluationContext(method, target, args, result);
    ...
    private EvaluationContext createEvaluationContext(JoinPoint joinPoint, Object result) {
        ...
        StandardEvaluationContext context = createEvaluationContext(method, joinPoint.getTarget(), joinPoint.getArgs(), result);
    ...
    private StandardEvaluationContext createEvaluationContext(Method method,
                                                              Object target,
                                                              Object[] args,
                                                              Object result) {
        ...
        bindIndexedArguments(context, safeArgs);
        if (method != null) {
            String[] parameterNames = PARAMETER_NAME_DISCOVERER.getParameterNames(method);
            if (parameterNames != null) {
                for (int i = 0; i < Math.min(parameterNames.length, safeArgs.length); i++) {
                    context.setVariable(parameterNames[i], safeArgs[i]);
                }
            }
        }

        context.setVariable("target", target);
        context.setVariable("method", method != null ? method.getName() : null);
        context.setVariable("args", safeArgs);
        context.setVariable("result", result);
        context.setVariable("joinPoint", null);
        if (target != null) {
    ...
    private void bindIndexedArguments(StandardEvaluationContext context, Object[] args) {
